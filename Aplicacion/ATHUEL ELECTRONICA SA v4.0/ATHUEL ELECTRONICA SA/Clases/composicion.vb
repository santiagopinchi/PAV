Public Class composicion
    ...
    Dim _nombre_tabla As String = "composicionProducto"
    ...
    Private Function transferir_a_la_clase(ByRef controles As DataGridView) As estado_transferencia
        ...
        For c = 0 To controles.Rows.Count - 1
            datos.Rows.Add()
            datos.Rows(c)("codTipoProdFinal") = controles.Rows(c).Cells(0).Value
            datos.Rows(c)("codProdFinal") = controles.Rows(c).Cells(1).Value
            datos.Rows(c)("orden") = controles.Rows(c).Cells(2).Value
            datos.Rows(c)("codTipoProdPar") = controles.Rows(c).Cells(3).Value
            datos.Rows(c)("codProdPar") = controles.Rows(c).Cells(4).Value
            datos.Rows(c)("descripcion_paso") = controles.Rows(c).Cells(5).Value
            datos.Rows(c)("fecha") = controles.Rows(c).Cells(6).Value
            datos.Rows(c)("cantidad") = controles.Rows(c).Cells(7).Value
        Next
    ...
    Public Function Insertar(ByVal i As Integer)
        ...
        Next
        sql += " INSERT " & Me._nombre_tabla & " (" & columnas & ") VALUES (" & valores & ") "
        Return sql
